...
const Navbar = () => {
    ...
    const [isOpen, setIsOpen] = useState(false); // état pour gérer si le menu est ouvert ou non
                ...
                <div className="navbar-start">
                    <div className="dropdown">
                        ...
                        <ul tabIndex={0} className={`dropdown-content mt-3 p-2 shadow bg-base-100 rounded-box w-52 ${isOpen ? 'block' : 'hidden'}`}>
                            {navItems.map(item => (
                                <li className="py-1 mt-3 mb-3" key={item.id}><NavLink to={item.link} onClick={() => setIsOpen(false)}>{item.name}</NavLink></li>
                            ))}
                        </ul>
                    </div>
                </div>
