...
function AdminPanel() {
  ...
  const toggleSelectedRecette = (recette) => {
    setSelectedRecette(
      selectedRecette && selectedRecette.id === recette.id ? null : recette,
    );
  ...
  const updateRecette = (id, newDetails) => {
    setRecettes(
      recettes.map((recette) =>
        recette.id === id ? { ...recette, ...newDetails } : recette,
      ),
    );
  };
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (Array.isArray(dataRecettes)) {
          ...
          console.error(
            "Data fetched for recettes is not an array:",
            dataRecettes,
          );
        }
      ...
      <div className="section">
        ...
        {recettes.map((recette) => (
          <div key={recette.id} className="item">
            <p onClick={() => toggleSelectedRecette(recette)}>
              {" "}
              {recette.title}
            </p>
            ...
            <div>
              <button
                className="update button-panel"
                onClick={() =>
                  updateRecette(recette.id, { title: "Nouveau titre" })
                }
              >
                Mettre à jour
              </button>
              <button
                className="delete button-panel"
